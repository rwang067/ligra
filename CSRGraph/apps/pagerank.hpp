...
void test_pagerank_pull(ChunkGraph* qsnap, int iteration_count) {
    ...
    if(prior_rank_array) free(prior_rank_array);
    if(dset) free(dset);
}

void test_pagerank_push(ChunkGraph* qsnap, int iteration_count){
    float* rank_array = 0 ;
	float* prior_rank_array = 0;
    float* dset = 0;
	vid_t v_count = qsnap->get_vcount();
