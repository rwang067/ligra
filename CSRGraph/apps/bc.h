...
static void ParallelLoadDegrees(ChunkGraph* snaph, pvector<int32_t> &degrees) {
...
void PBFS(ChunkGraph* snaph, vid_t source, pvector<vid_t> &path_counts,
          Bitmap &succ, std::vector<SlidingQueue<vid_t>::iterator> &depth_index,
...
pvector<ScoreT> run_bc(ChunkGraph* snaph, index_t _edge_count, sid_t root, vid_t num_iters) {
//  Timer t;
//  t.Start();
  pvector<ScoreT> scores(snaph->get_vcount(), 0);
  pvector<vid_t> path_counts(snaph->get_vcount());
  Bitmap succ(_edge_count);
  std::vector<SlidingQueue<vid_t>::iterator> depth_index;
...
void PrintTopScores(ChunkGraph* snaph, const pvector<ScoreT> &scores) {
  std::vector<std::pair<vid_t, ScoreT>> score_pairs(snaph->get_vcount());
  for (vid_t n=0; n<snaph->get_vcount(); n+=1)
    score_pairs[n] = std::make_pair(n, scores[n]);
  int k = 5;
  std::vector<std::pair<ScoreT, vid_t>> top_k = TopK(score_pairs, k);
  for (auto kvp : top_k)
...
bool BCVerifier(ChunkGraph* snaph, sid_t root, int32_t num_iters,
  ...
  for (int iter=0; iter < num_iters; iter++) {
    ...
    depths[source] = 0;
    std::vector<vid_t> path_counts(snaph->get_vcount(), 0);
    path_counts[source] = 1;
    std::vector<vid_t> to_visit;
...
//      }
    ...
    std::vector<std::vector<vid_t>> verts_at_depth;
    for (vid_t n=0; n<snaph->get_vcount(); n+=1) {
      if (depths[n] != -1) {
