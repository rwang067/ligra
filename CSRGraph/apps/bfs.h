...
int64_t BUStep(ChunkGraph* snaph, pvector<vid_t> &parent, GapbsBitmap &front, GapbsBitmap &next) {
...
int64_t TDStep(ChunkGraph* snaph, pvector<vid_t> &parent,
...
void BitmapToQueue(ChunkGraph* snaph, const GapbsBitmap &bm,
                   SlidingQueue<vid_t> &queue) {
#pragma omp parallel
  {
    QueueBuffer<vid_t> lqueue(queue);
...
pvector<vid_t> InitParent(ChunkGraph* snaph) {
...
pvector<vid_t> run_bfs(ChunkGraph* snaph, index_t _edge_count, sid_t root, int alpha = 15, int beta = 18)
{
...
void PrintBFSStats(ChunkGraph* snaph, const pvector<vid_t> &bfs_tree) {
  int64_t tree_size = 0;
  int64_t n_edges = 0;
