...
std::vector<std::pair<ValT, KeyT>> TopK_CC(
...
vid_t test_connected_components(ChunkGraph* qsnap, index_t neighbor_rounds = 2) {
    ...
    for (index_t r = 0; r < neighbor_rounds; ++r) {
        #pragma omp parallel 
        {
            ...
            for (vid_t u = 0; u < v_count; u++) {
                ...
                vid_t* local_adjlist;
                nebr_count = qsnap->get_out_degree(u);
                if (nebr_count == 0) continue;
                local_adjlist = new vid_t[nebr_count];
                local_degree  = qsnap->get_out_nebrs(u, local_adjlist);
    ...
    #pragma omp parallel 
    {
        ...
        for (vid_t u = 0; u < v_count; u++) {
            ...
            nebr_count = qsnap->get_in_degree(u);
            if (nebr_count == 0) continue;
            local_adjlist = new vid_t[nebr_count];
            local_degree  = qsnap->get_in_nebrs(u, local_adjlist);
            assert(nebr_count == local_degree);
    ...
    std::vector<std::pair<vid_t, vid_t>> top_k = TopK_CC(count_vector, k);
